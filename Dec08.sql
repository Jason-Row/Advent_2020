...
WHILE NOT EXISTS (SELECT SELECTED_ROW_ID FROM #TEMP WHERE SELECTED_ROW_ID = @row_id)
	BEGIN 
		...
		ELSE IF @operation = 'acc'
			BEGIN
				SET	@accumulator = @accumulator + @arguement
				SET @row_id = @row_id + 1
			END
		ELSE IF @operation = 'jmp'
			SET @row_id = @row_id + @arguement

	END

					PRINT concat('Accumulator = ', @accumulator)
...
DROP TABLE #TEMP
CREATE TABLE #TEMP (TRIAL_ID INT, SELECTED_ROW_ID INT, OPERATION_TXT NVARCHAR(3))
GO
...
WHILE @max_row_id <> 99999
	BEGIN
	SET @row_id = 1
	SET @trial_id = @trial_id + 1
	SET @change_row_id = @change_row_id + 1
	SET @accumulator = 0

		WHILE NOT EXISTS (SELECT SELECTED_ROW_ID FROM #TEMP WHERE TRIAL_ID = @trial_id and SELECTED_ROW_ID = @row_id)
			...
			BEGIN 
				...
				INSERT INTO #TEMP (TRIAL_ID, SELECTED_ROW_ID, OPERATION_TXT)
				VALUES (@trial_id, @row_id, @operation)


				IF @arguement <> 0
					IF @row_id = @change_row_id 
						IF @operation = 'nop' 
								SET @operation = 'jmp'
				...
				BEGIN
					IF @row_id = (SELECT MAX(ID_NUM) FROM ADVENT_DEC08_GAME) +1
						BEGIN
							SET @max_row_id = 99999
							BREAK
						END
					ELSE CONTINUE
				END			
